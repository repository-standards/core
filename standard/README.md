...
| [`scripts/`](scripts/) | Guards + tooling: `self-verify.mjs`, `spec-guard.mjs`, `spec-structure.mjs`, `schema-pair.mjs`, `facts-check.mjs`, `cycle-guard.mjs`, `changelog.mjs`, `verifyAgentGuards.sh`; [`scripts/spec/`](scripts/spec/) holds the spec engine's shared scripts + templates (MIT, `scripts/spec/LICENSE`) | as-is |
