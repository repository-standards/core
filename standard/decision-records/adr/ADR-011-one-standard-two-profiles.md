...
| | |
| --- | --- |
| **Status** | Accepted (2026-07-22 - owner directive; implemented: profile field in the manifest + self-verify --profile) |
| **Date** | 2026-07-22 |
...
- **C - A `profile` field on manifest entries, verified per profile (recommended).**
  ...
  pin; `self-verify --profile solo|team` counts drift against the right subset. One
...
`--profile solo` shows drift 0 while visibly not carrying scale-only artifacts.
