...
if (manifest && exists("stack.manifest.json")) {
  try {
    ...
    note("stack", `stack manifest present: ${stack.technology || "unnamed"} ${stackVersion} - technology layer counted in the same drift number (ADR-016/022)`);
    if (!stack.version)
      warning("stack", "stack.manifest.json declares no version - nothing records which state of the stack this repo aligned to");
...
if (!skeleton) {
  ...
  const PLACEHOLDER_RE = /<(\p{L}[\p{L}\p{N} '._+-]{0,58})>|^\|(?:\s*(?:\.{3}|…)\s*\|)+\s*$/gmu;
