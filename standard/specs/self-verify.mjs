...
//      coordinates people. --profile solo checks core only; --profile team (default, and
//      no flag) checks everything. An entry with no profile counts as core, so manifests
//      from before ADR-011 still check in full either way.
...
//   node scripts/self-verify.mjs --profile solo   # core-profile entries only (ADR-011)
//
...
const soloProfile = (pIdx >= 0 ? args[pIdx + 1] : "team") === "solo"; // team (or no flag) = everything
...
let scaleSkipped = 0; // entries skipped by --profile solo, across files/sections/guards

if (manifest) {
  // files
  for (const f of manifest.files || []) {
    if (soloProfile && !isCore(f)) { scaleSkipped++; continue; }
  ...
  for (const s of manifest.sections || []) {
    if (soloProfile && !isCore(s)) { scaleSkipped++; continue; }
  ...
  for (const g of manifest.guards || []) {
    if (soloProfile && !isCore(g)) { scaleSkipped++; continue; }
  ...
  if (soloProfile && scaleSkipped > 0) {
    note("profile", `${scaleSkipped} scale-only entr${scaleSkipped === 1 ? "y" : "ies"} skipped (--profile solo)`);
  }
