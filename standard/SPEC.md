...
Version 0.8.11 - the spec is versioned with the standard (`VERSION`). **The standard is
...
- **R25.** A PR that changes what the standard ships MUST move the version, and the
  ...
  stops meaning anything. The patch position is not a two-digit field: 1.0.11 is
  followed by 1.0.11, then 1.0.110 - each larger than the last. Versions mark this
