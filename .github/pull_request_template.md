...
- [ ] The gate set is green locally: `node tools/tree-check.mjs`, `node tools/link-check.mjs`, `node standard/scripts/spec-structure.mjs`, `node standard/scripts/spec-guard.mjs --audit`, `node tools/spec-guard-test.mjs`, `node tools/schema-pair-test.mjs`, `node tools/facts-check.mjs`, `node tools/facts-check-test.mjs`, `node tools/docsite.mjs && node tools/site-check.mjs`
