...
1. **Scaffold + pin.** Bring in the shipped tree (the align steps in `SKILL.md`), write
   `.standards-version` and carry the manifest. Instantiate `specs/constitution.md` from
   `specs/constitution.template.md` - the project's non-negotiables the spec skills gate
   on. Choose and add a LICENSE now (an unlicensed repo is unusable, not neutral) and
   fill `SECURITY.md`'s contact. Empty but valid: `self-verify` passes.
