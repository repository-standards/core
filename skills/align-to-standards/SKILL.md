...
- **The delta is measured, not curated.** The work list is what `self-verify` reports open.
  Ordering a wave is judgment; **what is in the plan at all is not** - an entry does not
  leave the list because this run classified it as belonging to the standard's own repo
  rather than to this one. The boundary is already drawn, and it is not the agent's to
  redraw: transition skills do not ship and this skill is the one that is left (ADR-009 -
  `self-verify` flags any of them found in an adopted repo as stray); everything under
  `standard/.claude/skills` ships and is part of the delta - `record-run` included, which
  is a lifecycle skill by ADR-045 and not this repo's own tooling.
- **Hand-hold, do not dump.** For each wave item, guide the user through it (elicit,
  ...
  dropped - and the record is the number, not a sentence in this session: a deferred item
  stays open drift that the next run re-reads from measurement. Every open entry is either
  in a wave or named to the user as still open; a third state ("consciously skipped",
  living only in the agent's prose) is what makes a dropped item invisible once the session
  ends. Taking something out of the count for good is a different act with a different
  cost: a manifest exception the user approves - a line in the repo carrying the reason,
  which lowers the adoption percentage rather than hiding the gap (R17).
