#!/usr/bin/env node
// sprint-guard-test - drive the shipped work-sprint guard over real fixtures.
//
// The guard's whole job is one invariant: an intent is in the backlog pool or in exactly
// one sprint. It reports nothing when a repo has no sprints, which is correct behaviour and
...
//
// Usage: node tools/sprint-guard-test.mjs   # exit 1 on any failure
...
const GUARD = join(process.cwd(), "standard/scripts/sprint-guard.mjs");
const TEMPLATE = join(process.cwd(), "standard/docs/sprints/_template.md");
...
// A sprint file, as `sprint-open` writes one: the rows live under `## Intents`, because that
// heading is what the guard scopes to. A file without it yields no rows, and no rows reads
// exactly like a clean sprint - so these helpers exist to keep the fixtures honest about the
// shape, and the cases below assert that the missing heading is an error rather than a pass.
const sprint = (...ids) => `# Sprint\n\n## Intents\n\n${table(...ids)}`;
const cycleStatus = (...pairs) => `# Sprint\n\n## Intents\n\n${statusTable(...pairs)}`;

// A closed sprint carrying an `## Outcome` block, for the returned-id checks.
const closedCycle = (ids, outcome) =>
  `# Sprint\n\n| | |\n| --- | --- |\n| **Status** | closed |\n\n## Intents\n\n${table(...ids)}\n## Outcome\n\n${outcome}\n`;

// A sprint whose header table declares a status, for the in-flight pointer checks: a pointer
// row is stale exactly when the sprint it names has closed.
const statedCycle = (state, ...ids) =>
  `# Sprint\n\n| | |\n| --- | --- |\n| **Status** | ${state} |\n\n## Intents\n\n${table(...ids)}`;

// The pool's `## In flight` table - team, goal, target, the sprint file and how many intents
// it holds. `sprints/...` is written relative to the pool's own directory, the way both the
...
// files: { "docs/backlog.md": "...", "docs/sprints/team/x.md": "..." }
const fixture = (files) => {
  const dir = mkdtempSync(join(tmpdir(), "sprint-guard-"));
...
const CYC_A = "docs/sprints/payments/august.md";
const CYC_B = "docs/sprints/growth/august.md";

check("a repo with no sprints directory is not using sprints", {
  files: { [POOL]: table("SPEC-1", "SPEC-2") },
  code: 0,
  expect: ["no sprints in use"],
});

check("pool and one sprint, no overlap", {
  files: { [POOL]: table("SPEC-1", "SPEC-2"), [CYC_A]: sprint("PAY-9") },
  code: 0,
  expect: ["OK", "3 intent(s)"],
});

check("an intent in the pool and a sprint at once fails", {
  files: { [POOL]: table("SPEC-1", "PAY-9"), [CYC_A]: sprint("PAY-9") },
  code: 1,
  expect: ["PAY-9 is in 2 places", "belongs to the pool or to one sprint"],
});

check("the same intent in two teams' sprints fails", {
  files: { [POOL]: table("SPEC-1"), [CYC_A]: sprint("PAY-9"), [CYC_B]: sprint("PAY-9") },
  code: 1,
  expect: ["PAY-9 is in 2 places"],
});

check("every violation is reported, not just the first", {
  files: { [POOL]: table("A-1", "B-2"), [CYC_A]: sprint("A-1", "B-2") },
  code: 1,
  expect: ["A-1 is in 2 places", "B-2 is in 2 places"],
});

check("advisory without --block: reports and exits 0", {
  files: { [POOL]: table("PAY-9"), [CYC_A]: sprint("PAY-9") },
  block: false,
  code: 0,
  expect: ["PAY-9 is in 2 places"],
});
...
check("a sprint with no rows yet is valid", {
  files: { [POOL]: table("SPEC-1"), [CYC_A]: "# Sprint\n\n## Intents\n\nnothing pulled in yet.\n" },
  code: 0,
  expect: ["OK"],
});

// The heading was seen even though nothing follows it, so this is an empty sprint rather than
...
check("a sprint whose Intents heading is the last line is empty, not unreadable", {
  files: { [POOL]: table("SPEC-1"), [CYC_A]: "# Sprint\n\n## Intents" },
  code: 0,
  expect: ["OK"],
});

// The shipped backlog template, verbatim - its in-flight pointer table lists sprints, not
// intents, and its example row must not read as one.
{
  const dir = fixture({ [CYC_A]: sprint("PAY-2", "PAY-3") });
...
check("the backlog at its primary manifest path is still read", {
  files: { "backlog.md": table("PAY-9"), [CYC_A]: sprint("PAY-9") },
  code: 1,
  expect: ["PAY-9 is in 2 places"],
});

check("sprints with no backlog anywhere is reported, not passed", {
  files: { [CYC_A]: sprint("PAY-9") },
  code: 1,
  expect: ["no backlog at", "cannot be checked"],
});

check("the derived timeline is not a sprint", {
  files: { [POOL]: table("SPEC-1"), [CYC_A]: sprint("PAY-9"), "docs/sprints/TIMELINE.md": table("PAY-9") },
  code: 0,
  expect: ["OK"],
});

check("a template directory is skipped like a template file", {
  files: { [POOL]: table("PAY-9"), "docs/sprints/_drafts/x.md": table("PAY-9") },
  code: 0,
  // Skipping `_drafts/` leaves no sprint files at all, so the guard takes the
  // not-using-sprints branch rather than reporting OK. Either way PAY-9 is not duplicated,
...
check("a table inside a fenced block is illustration, not rows", {
  files: { [POOL]: table("PAY-9"), [CYC_A]: "# Sprint\n\n## Intents\n\n```\n" + table("PAY-9") + "```\n" },
  code: 0,
  expect: ["OK"],
});

// The shipped sprint template, instantiated the way sprint-open tells a user to instantiate
// it - at a real sprint path, without the `_` prefix. The earlier version of this case
...
// "comment" was not one.
{
  ...
  mkdirSync(join(dir, "docs/sprints/payments"), { recursive: true });
  cpSync(TEMPLATE, join(dir, "docs/sprints/payments/august.md"));
  const { code, out } = run(dir, true);
  rmSync(dir, { recursive: true, force: true });
  if (code !== 0) {
    failures++;
    console.error(`  FAIL the shipped template, instantiated as a real sprint, does not trip the guard\n       exit ${code}: ${out.trim()}`);
  } else {
    console.log("  ok    the shipped template, instantiated as a real sprint, does not trip the guard");
  }
}
...
check("a block reaches across the pool into a sprint", {
  files: {
...
check("a sprint with no Intents heading is an error, not an empty sprint", {
  files: { [POOL]: table("PAY-9"), [CYC_A]: `# Sprint\n\n${table("PAY-9")}` },
  code: 1,
...
// The folder manual is what an author reads before writing a sprint by hand, and it
...
{
  const manual = readFileSync(join(process.cwd(), "docs/tree/docs-sprints.md"), "utf8");
  const example = manual.match(/```markdown\n([\s\S]*?)```/);
  if (!example) {
    failures++;
    console.error("  FAIL docs/tree/docs-sprints.md has no ```markdown sprint example to check");
  } else {
    ...
    if (code !== 0 || !out.includes("3 intent(s)")) {
      failures++;
      console.error(`  FAIL the documented sprint format is what the guard reads\n       exit ${code}: ${out.trim()}`);
    } else {
      console.log("  ok    the documented sprint format is what the guard reads");
    }
  }
}
...
check("a returned-to-pool line only matters when the sprint is closed", {
  files: {
    [POOL]: table("PAY-1"),
    [CYC_A]: `# Sprint\n\n| | |\n| --- | --- |\n| **Status** | open |\n\n## Intents\n\n${table("PAY-2")}\n## Outcome\n\nReturned to the pool: PAY-9\n`,
  },
...
// `split:<id>` - the sprint-boundary answer (ADR-029). It is finished work, so a block on it
...
check("one intent copied into a sprint and renumbered in the pool is caught", {
  files: {
...
// and an author who keeps the wording is doing what `/sprint-close` told them to.
...
// The pool's in-flight pointer table. `/sprint-open` writes the row and `/sprint-close`
...
check("a pointer row for a sprint that has closed is stale", {
  files: {
    [POOL]: `${inFlight(["dispatch", "sprints/dispatch/may.md", 2])}${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("closed", "DSP-1", "DSP-2"),
  },
  code: 1,
  expect: ["still points dispatch at docs/sprints/dispatch/may.md, which is closed"],
});

check("a pointer row for an open sprint with a matching count passes", {
  files: {
    [POOL]: `${inFlight(["dispatch", "sprints/dispatch/may.md", 2])}${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1", "DSP-2"),
  },
  code: 0,
  expect: ["OK"],
});

check("a pointer row whose item count disagrees with the sprint is caught", {
  files: {
    [POOL]: `${inFlight(["dispatch", "sprints/dispatch/may.md", 4])}${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1", "DSP-2"),
  },
  code: 1,
  expect: ["says dispatch's sprint holds 4 item(s), but docs/sprints/dispatch/may.md holds 2"],
});

check("a pointer row whose item cell is not a bare count is reported as unreadable", {
  files: {
    [POOL]: `${inFlight(["dispatch", "sprints/dispatch/may.md", "2 items"])}${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1", "DSP-2"),
  },
  code: 1,
...
check("a pointer row naming a sprint file that is not there is caught", {
  files: {
    [POOL]: `${inFlight(["dispatch", "sprints/dispatch/june.md", 2])}${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1", "DSP-2"),
  },
  code: 1,
  expect: ["is running `sprints/dispatch/june.md`, but no sprint file is there"],
});

// Deleting the pointer row rather than fixing it would otherwise be the cheapest way past
// every check above.
check("an open sprint no pointer row names is caught, once the pool keeps the table", {
  files: {
    [POOL]: `${inFlight(["dispatch", "sprints/dispatch/may.md", 2])}${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1", "DSP-2"),
    "docs/sprints/growth/may.md": statedCycle("open", "GRO-1"),
  },
  code: 1,
  expect: ["docs/sprints/growth/may.md is open, but no in-flight row"],
});

// A table with no pointer rows in it is a pool not running sprints from its own side, rather
...
check("a pool with the table present but no pointer rows is not using it", {
  files: {
    ...
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1"),
  },
  code: 0,
  expect: ["OK"],
});
...
// the tree and opens a sprint before touching the table must not be failed for it.
{
  const dir = fixture({ "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1") });
  cpSync(join(process.cwd(), "standard/docs/backlog.md"), join(dir, POOL));
  const { code, out } = run(dir, true);
  rmSync(dir, { recursive: true, force: true });
  if (code !== 0) {
...
check("a closed sprint needs no pointer row", {
  files: {
    [POOL]: `${inFlight(["dispatch", "sprints/dispatch/may.md", 1])}${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1"),
    "docs/sprints/growth/april.md": statedCycle("closed", "GRO-1"),
  },
  code: 0,
  expect: ["OK"],
});

check("an in-flight row this guard cannot key on a sprint is reported, not skipped", {
  files: {
    [POOL]: `# Backlog\n\n## In flight\n\n| Team | Goal | Items |\n|---|---|---|\n| dispatch | a goal | 2 |\n\n## Epic: everything\n\n${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1", "DSP-2"),
  },
  code: 1,
  expect: ["an in-flight row with no `sprint` column to read"],
});
...
check("a pointer row with the sprint cell blanked is reported, not skipped", {
  files: {
...
check("a pointer written as a markdown link resolves the same as a bare path", {
  files: {
    [POOL]: `# Backlog\n\n## In flight\n\n| Team | Goal | Target | Sprint | Items |\n|---|---|---|---|---|\n| dispatch | a goal | 2026-09-01 | [may](sprints/dispatch/may.md) | 2 |\n\n## Epic: everything\n\n${table("SPEC-1")}`,
    "docs/sprints/dispatch/may.md": statedCycle("open", "DSP-1", "DSP-2"),
  },
  code: 0,
  expect: ["OK"],
...
console.log();
if (failures) {
  console.error(`sprint-guard-test: ${failures} case(s) failed`);
  process.exit(1);
}
console.log(
  "sprint-guard-test: OK - one intent lives in exactly one place under one title, blocks and splits point at something real, a sprint the guard cannot read is an error, a return actually lands, and the pool's in-flight pointers agree with the sprints they name",
);
