...
| [prose-check.mjs](prose-check.mjs) | markdown that renders as something other than what it says - one rule so far, a hyphen wrapping to the start of a line and becoming an orphan bullet, which had already shipped on two pages before the check existed |
...
| [validation.mjs](validation.mjs) | renders the published validation pages from `docs/validation/`'s own data, so no number on them can drift from the rows behind it; `--check` fails CI on a stale render or an unwaived open failure |
