...
| [skill-format-check.mjs](skill-format-check.mjs) | every skill this repo ships is still a conforming Agent Skill - frontmatter shape, the name matching its own directory, the description inside the format's limit, and no frontmatter key the format does not define. `ATTRIBUTIONS.md` claims that conformance in public, and a claim nothing checks is one the next skill written quietly breaks. The recommended instruction budget is a **ratchet** rather than a rule: the three skills over it today are recorded with the size they were, an allowance may only shrink, and a skill that comes back inside the budget fails until its line is deleted |
