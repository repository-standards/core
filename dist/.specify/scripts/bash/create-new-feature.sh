...
# PATCHED(repository-standards): ADR-002 capability paths - feature dirs are
# specs/<slug>/ with NO numeric or timestamp prefix, and the branch name is the
# slug itself. --number and --timestamp are accepted but ignored; FEATURE_NUM is
# kept (empty) only so the output shape stays compatible with upstream consumers.
if [ "$USE_TIMESTAMP" = true ] || [ -n "$BRANCH_NUMBER" ]; then
    >&2 echo "[specify] Warning: --number/--timestamp are ignored (capability paths: specs/<slug>/, no prefix)"
fi
FEATURE_NUM=""
...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    # PATCHED(repository-standards): ADR-002 - no numeric prefix, so the whole
    # budget belongs to the slug and the truncated name is the slug alone.
    PREFIX_LENGTH=0
...
if [ "$DRY_RUN" != true ]; then
    ...
    if [ ! -f "$SPEC_FILE" ]; then
        # PATCHED(repository-standards): the spec shape is owned by the standard -
        # instantiate from specs/capability-spec.template.md (Spec tier / Serves /
        # Status / Success metric), not from a vendored spec-template.md (not shipped).
        TEMPLATE="$REPO_ROOT/specs/capability-spec.template.md"
