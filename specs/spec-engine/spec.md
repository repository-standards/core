...
- **A name miss MUST NOT be treated as evidence that a capability is new.** `/spec-specify` derives the directory name from the words of the request, so a request about behaviour an existing capability already owns collides with no existing directory when it is phrased differently - and the existing "if `specs/<short-name>/` exists, update it in place" check passes it straight through to minting. Before creating a directory the skill MUST look for an existing owner (the capability specs' Purpose and Scope, or `/spec-impact`) and route to `/spec-update` when it finds one, naming the capability it matched. An approved idea is the routine case: its slug names the idea, never the capability, and handing that slug to specify produced `specs/<idea-slug>/` beside the spec that already described the behaviour it changed - two specs for one capability, the older one still stating what was replaced (R8).
