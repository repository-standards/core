...
- **Decision intake** (ADR-033) - the precedence the loop reads by: **record -> spec -> dossier -> user**. An `Accepted` record in scope is the answer, not a question, and outranks what the user says in the moment; `Proposed`/`Rejected`/`Superseded` bind nothing. The duty this places on specify and clarify is under Requirements.
- **Discovery intake** (ADR-024) - specify/clarify/plan/**impact** read the topic's dossier under `docs/discovery/` before asking the user: only entries newer than the dossier's `Last reconciled:` stamp are questions; a dossier is never normative (the spec has already won), and consuming a dossier marks its entries `folded-into-spec` and moves the stamp. The dossier itself is curated by the separate `discovery-digest` skill, which never writes specs.
...
- **The loop reads the decision log before it writes** (ADR-033). `/spec-specify` MUST read `docs/decision-records/`'s
  index before drafting and `/spec-clarify` MUST read it before asking, opening in full only the records whose subject
  overlaps. Reading the index *instead of* the directory is sound only because `decision-records-check` fails a record
  on disk the index omits; where that guard is not running the directory MUST be listed too - the fixture proved the
  case, its BDR index stopping at 003 while the record governing the request was 004. What an **Accepted** record
  settles enters the spec as settled content citing it, never as a question and never as a DECISION marker. A request
  or an answer that contradicts one MUST stop the run naming the record, with exactly two routes offered - change the
  request, or supersede the record (R6). Neither skill mentioned
  `docs/decision-records/` at all; both named the dossier as the sole pre-user answer source, and a dossier is never
  normative - so the loop's entry point consulted the source that binds nothing and never the one that binds everything.
- **A capability is matched by subject, not by slug** (ADR-033). `/spec-specify` MUST list `specs/` and match on what the
  capability is about before minting a directory: the short name it generated is its own, not the repo's, so
  `shift-reminders` and `shift-notifications` are one capability under two names. Matching on the generated slug alone
  gives one capability two rival specs, and gives a retired capability a way to be reopened that nobody decided on.
- **A retired capability stops the PO's entry point too, not only the developer's** (ADR-033). `/spec-specify` MUST read
  the matched spec's `Status` at the step that decides where to write, and MUST stop before drafting on `retired`,
  naming the record that ended it and handing the call back to the user. The stop is surface-and-hand-back, not refuse:
  a new capability specced fresh is frequently the right answer and the retiring record often says so - but the agent
  MUST NOT proceed alone, under the old slug or a new one. The check existed in `/spec-impact`, `/spec-update` and
  `discovery-digest` and was absent from the one entry point the product points a PO at, whose documented behaviour for
  an existing directory was to update it in place.
...
- **The plan's source layout MUST be one the capability map can bind.** `plan-template.md`'s default single-project structure is sliced by capability, matching the decision checklist's own paved road ("slice by capability/domain, not by layer or page"), and the Structure Decision field MUST tell the author to bind the chosen paths in `specs/capability-map.json` and verify with `spec-guard --audit`. Upstream's layer-folder default (`models/`, `services/`, `cli/`, `lib/`) is the layout a domain-named glob cannot see: a capability implemented across those folders left every glob matching nothing and the coupling guard reporting OK on code whose spec was never touched.
- **Capability collision MUST be checked by behaviour, not by slug.** `/spec-specify` MUST read the existing capability directories and map keys - their Purpose, Scope and Out of scope - before minting a directory, and MUST report what it compared. An exact-string test on the generated slug catches only the easy half: the same behaviour arrives worded as the actor, the surface or the ticket, so `manager-shift-reassignment` minted a sibling beside an existing `shift-swap-request` that already owned it. A request spanning two capabilities MUST produce one spec plus a typed DECISION marker naming the boundary and the other capability - never a silent absorption and never two specs from one invocation. This is how the step surfaces a boundary call while keeping the rule that specify does not ask: the marker blocks the gate, and clarify owns the question. A match routes to `/spec-update` on the capability it matched, named in the report, rather than to a new directory: an approved idea is the routine case, because its slug names the idea and never the capability, and handing that slug to specify produced `specs/<idea-slug>/` beside the spec that already described the behaviour it changed - two specs for one capability, the older still stating what was replaced (R8).
- **The clarify record has a fixed position in the spec.** `capability-spec.template.md` MUST mark where `## Clarifications` goes - immediately after `## Purpose`, before `## Scope` - and `/spec-clarify` MUST create it there and nowhere else, with every later session a `### Session YYYY-MM-DD` under the one heading. "Just after the highest-level contextual section" was a judgement over sixteen headings: two runs could place it differently, which is churn in a file whose section order specify separately requires be preserved, and the run that guesses differently writes a second heading the gate cannot see past. The template MUST NOT ship the heading itself - the gate greps for it, so a template carrying it would grant that check to a spec where no clarify session ever ran.
- **A retired spec is frozen against extension, not against correction** (ADR-036, R4). `/spec-impact` MUST check `Status` on every capability it reaches, ripple targets included, and report a retired ripple target as a correction target rather than passing over it. `/spec-update` MUST stop before *extending* a retired spec and MUST correct a statement this change makes false, in this change's pull request, bounded to what the capability did plus what superseded it, with `Status` left at `retired`. `/spec-reconcile` MUST resolve a contradiction involving a retired spec in one direction: the live spec wins and the retired one is corrected. The three previously disagreed - stop, resolve, and never detect it - which left a retired spec stating something provably false with no legal way to fix it.
- **`/spec-impact`'s ripple MUST reach the artifacts a change falsifies outside `specs/`.** Other capability specs, the decision records and code were the whole search, so a change could contradict a runbook, a persona's stated job, a product scope claim and a backlog row's definition of done with every named check reporting clean. The step MUST take each artifact the repo keeps - `docs/runbooks/`, `docs/personas.md`, `docs/PRODUCT.md`, `docs/analytics.md` and the backlog - and produce a verdict per artifact: the sentence falsified, or untouched. The search keys on the capability's own terms, because a sentence that contradicts the change is a sentence that names the thing being changed; it is not a re-read of every document.
...
- **`checklists/requirements.md` belongs to the spec, not to the piece of work.** `/spec-specify` writes it while minting the spec and `/spec-clarify` re-validates it on every later round, so it survives the close, and `spec-structure` MUST NOT ask for its removal - R13 and ADR-010 make `plan.md` and `tasks.md` ephemeral and name nothing else. Four skills touched this file and disagreed about what it was: following the documented order produced a warning to delete it one command after the standard's own skill created it, while the only step that deletes anything never removed it.
- **`/spec-update` MUST close the discovery loop it consumed.** The documented route for material about an already-shipped capability is `discovery-digest -> /spec-impact -> /spec-update`, so the stamp has to move at the end of that route as well as at mint time; without it a capability specced once and changed afterwards reads `Last reconciled: never` for the rest of its life and re-raises every entry under the stamp as fresh material on every round. `/spec-impact` names the unreconciled entries in its output; `/spec-update` marks the consumed ones `folded-into-spec`, strikes the contradiction rows the new spec text settles, and sets the stamp - and leaves the stamp untouched when it folded nothing in, because advancing it past unread material is how a question stops being asked without being answered.
...
- **`/spec-reconcile` MUST record a compatibility artifact as one, never as current design.** Where the source of truth for some behaviour is already shipped - a previous release's compiled binary, a wire format in the field, a published URL - the code preserving it is not what the capability is, and the step's standing preference (make the spec accurate to the real behaviour) files it under current design if nothing says otherwise. It gets its own subsection naming what it preserves and the condition for removing it, with the capability's contracts left describing the current shape; deleting it from the spec is the opposite error, because it ships. What counts as a breaking change stays the repo's own recorded decision, not something this step settles in passing.
...
- **`/spec-impact`'s ripple MUST read both decision-record streams, not the ADRs alone.** The step named `ADRs` while the standard, and this same skill one step later, distinguish ADR from BDR - so a change an Accepted BDR explicitly forbids was analysed without `docs/decision-records/bdr/` ever being opened, and reported as no decision impact. What a change may **not** do is typically a business constraint (a licence boundary, a vendor or customer contract, a regulatory limit, a persona call), stated in a BDR's `What this rules out` - the only section that carries a capability's non-goals. Distinct from the `retired` check in step 1, which reads the primary spec's own `Status`: a live capability can still have a forbidden extension.
...
- **A record answers instead of the user.** GIVEN an Accepted record that settles a question the spec leaves open WHEN `/spec-clarify` builds its queue THEN that question is excluded and the record's answer is written into the spec citing it, rather than asked.
- **A colliding request stops before the draft.** GIVEN a request that contradicts an Accepted record WHEN `/spec-specify` runs THEN no spec is drafted, the record is named, and the two routes offered are changing the request or superseding the record - never writing the spec anyway.
- **A colliding answer is a supersession.** GIVEN the user answers a clarify question in a way that contradicts an Accepted record WHEN the answer is processed THEN it is not written into the spec; the collision is named and routed to `/adr-write` or `/bdr-write`.
- **Retirement stops the PO's entry point too.** GIVEN `specs/shift-notifications/` with `Status: retired` linking the BDR that ended it WHEN `/spec-specify` runs for a request that would extend it THEN it stops before drafting, names that record, and hands the call back to the user - it neither updates the retired spec in place nor mints a fresh directory on its own.
- **A near-miss slug does not mint a rival spec.** GIVEN `specs/shift-notifications/` exists and specify generates the short name `shift-reminders` WHEN the directory is resolved THEN `specs/` is listed and the existing capability is matched on subject, so the request resolves to that capability rather than to a second spec for the same subject under a different name.
- **The index read is guarded, not trusted.** GIVEN a repo whose decision-record README omits a record present on disk WHEN the decision intake runs THEN the omission is a `decision-records-check` failure, and where that guard is not running the skills list the directory rather than trusting the table - reproduced on the fixture, whose BDR index stops at 003 while BDR-004 governs the request.
...
- **An existing owner under a different name.** GIVEN `specs/shift-swap-request/` owns the approval behaviour and an approved idea slugged `swap-approval-rules` changes it WHEN specify's directory resolution runs THEN the existing capability is named as the owner and the change routes to `/spec-update`, and no `specs/swap-approval-rules/` directory is minted.
- **A retired ripple target is a correction target.** GIVEN a change to a live capability that makes a statement in a retired capability's spec false WHEN `/spec-impact` runs THEN the retired spec is reported with its `Status` and named as a correction target rather than skipped; WHEN `/spec-update` then runs THEN that statement is corrected to what the retired capability did plus what superseded it, `Status` stays `retired`, and no behaviour is added to it.
- **The ripple leaves the specs directory.** GIVEN a change that contradicts a runbook step, a persona's stated job, a `docs/PRODUCT.md` claim and a backlog row's definition of done WHEN `/spec-impact` runs THEN each of the four is named with the line it falsifies, rather than the output listing only specs, records and code areas.
...
- **The quality checklist is not scaffolding.** GIVEN a spec directory carrying `checklists/requirements.md`, created by `/spec-specify`'s own step, WHEN `spec-structure` runs THEN it is not named as scaffolding to remove; GIVEN the same directory carrying `plan.md` or `tasks.md` after the work closed THEN each still is.
- **The change path moves the stamp.** GIVEN a dossier entry newer than the stamp, routed `discovery-digest -> /spec-impact -> /spec-update`, and a spec edited in that session to resolve it WHEN `/spec-update` completes THEN the entry reads `folded-into-spec`, the contradiction it raised is struck, and the stamp names today's date and the spec; GIVEN the same route where nothing was folded in THEN the stamp is unchanged.
