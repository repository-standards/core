...
| check-spec-clarified.sh | 1 | no argument; spec file not found; no `## Clarifications`; >0 open `NEEDS`-family markers; >0 family names written as a list item instead of brackets; or >0 marker-shaped tokens the gate does not recognise (each listed with line numbers, stderr) |
