...
6. **Profiles.** `--profile core` checks core entries only across files/sections/guards and notes how many scale-only entries were skipped; `--profile scale` checks everything. With no flag, the repo's manifest copy's top-level `profile` field (written at align time, ADR-011) is the default - a note names it - and absent that, `scale`. `solo`/`team` are accepted as deprecated aliases. A manifest copy carrying **no** `profile` at all raises a WARN naming the fallback: the shipped manifest declares the field, the shipped CI gate reads the same one, and falling back silently put every such repo on the scale-tier gate - including the solo repos the core profile exists for - while R11's own *(scale)* scoping had nothing acting on it. The fallback itself stays: the stricter tier is the right direction to fail.
