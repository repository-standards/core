...
| `VERSION` | text, one line | this repo's own: `x.y.z`, the version the standard currently ships. Read for the changelog-heading and `since` checks; the shipped tree is checked for **not** restating it. |
...
4. **Version surface.** `standard/SPEC.md` must **not** open a line with `Version <x.y.z>` - any such line FAILs, quoted back. The header used to be required to equal `VERSION`, and the assertion is inverted rather than dropped ([ADR-056](../../docs/decision-records/ADR-056-the-release-tag-is-made-by-ci-and-the-version-is-stated-once.md)): the shipped tree's only working copy of the version is the manifest's `version`, which `self-verify` compares against an adopter's `.standards-version`. The header was read by nothing, and it made a shipped file's hash move on every release, which put each version cut through the `verify-engine` coupling guard. What is refused is the **pattern**, not today's number, because `Version 1.0.1` left behind by a bump is precisely the defect the old check existed for; it is anchored to the line start so that SPEC.md's own prose about version ordering is discussion rather than a restatement. The README is likewise **not** checked for `@<V>`: that assertion required the quick start to instruct an adopter to pin a version, which is the model [ADR-025](../../docs/decision-records/ADR-025-the-standard-is-living-latest-is-the-target.md) removed. A guard that demands phrasing a decision deleted keeps reinstating it, and this one did - it failed the moment the README was corrected.
...
- The shipped spec MUST NOT restate the version. A release then moves `VERSION`, the changelog section and the manifest's `version` field, and touches no shipped file whose hash a coupled capability watches.
...
- **A version in the spec header.** GIVEN `standard/SPEC.md` opens a line `Version 0.7.2` WHEN tree-check runs THEN it FAILs quoting that line and exit is 1 - whether or not `0.7.2` is what `VERSION` says, since a stale restatement is the case the rule is for and matching the current number would let it through.
