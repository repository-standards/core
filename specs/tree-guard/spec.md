...
Guard the single authored shipped tree (`standard/`, ADR-014): nothing repo-own leaks in, everything the manifest promises is present, and the tree passes its own verifier. Plus: no relative markdown link anywhere in the repo is dead.
...
- tree-check MUST run all four checks and report every failure, never stop at the first.
