...
| SKILL-COUNT-1 | open-question | Shipped skills | Decided: discriminability is the constraint, not the count - 21 ship today (per `AUTHOR-1`) | the doubt: four sibling authoring skills could drift apart. [`shipped-skills`](docs/open-questions/shipped-skills.md) | decided |
...
| OWN-SPECS-1 | open-question | The repo specs its own tooling | Decided: four buildable capability specs in `specs/` | the doubt: specs for small scripts risk documentation theatre. [`own-specs`](docs/open-questions/own-specs.md) | decided |
