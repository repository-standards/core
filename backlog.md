...
| ENG-2 | Manifest + align-engine | `align-to-standards` is prose; a data-driven manifest with versioned migrations makes reconcile measurable and repeatable | a manifest describes what a repo must have; the engine diffs + applies; drift is a number | todo |
| ENG-3 | ADR: "align-engine is a manifest" | the ENG-2 shape is a re-litigable decision worth recording | ADR drafted with rejected alternatives once ENG-2 is designed | blocked (on ENG-2) |
