...
| ENG-5 | Vendor the patched engine | ADR-013 decided the posture; the copy itself needs a real upstream fetch + testing | fetch spec-kit agent-side assets, patch (paths/gate/our-templates), ship, verify end-to-end | done (this PR): v0.13.2 vendored under `standard/spec-kit/` (4 marked patches; scratch tests green: capability paths without prefixes, gate blocks/passes, specs instantiated from OUR templates, unpatched files byte-identical upstream) and shipped **as skills** per the owner's simplification call - `dist/.claude/skills/speckit-*` x10 with per-file provenance notes beside the nine lifecycle skills, shared runtime + MIT license in `dist/.specify/`; no upstream CLI needed | done (this PR) |
