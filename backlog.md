...
| id | title | why | DoD | status |
|----|-------|-----|-----|--------|
| FIELD-1 | Run the standard against three real repositories | the align router is the product's core and the only major component with no gate of any kind - every mechanical guard has a test, the router has none. Until it has been run against a repo nobody wrote for it, "walks a messy repo back to health" is a design claim | three public repos aligned as a user would ask, not as an author would drive: one small on a registered stack, one mid-size on an **unregistered** stack (the honest-miss path, `stack-decisions.md`, the consent-gated issue), one large and multi-year (waves, a counted backlog, drift as a number). Five command shapes exercised - align, plan-only, greenfield, registry miss, verify - with real ADRs and BDRs written along the way. Findings land as backlog items or fixes; the run itself is written up as the adoption evidence EXHIBIT-1 asks for | todo - [`docs/validation/`](docs/validation/README.md) now carries a large **L1** (read-only assessment) evidence base - 105 targets, 103 real repositories, 174 cases, 176 observations, 78 fixed-and-re-verified failures across 18 merged pull requests, 84 still open (2 of them an attempted fix a re-run found did not fully hold) - but every one of those repos was assessed, never aligned. What this item asks for is depth **L2/L3**: the align router's decisions worked out for real against a repo nobody on this project wrote, with an actual drift number and real ADRs/BDRs, not a dry read. That gap is unchanged, and is the reason this item stays todo rather than done |
...
| EXHIBIT-1 | An adoption you can point at | the README claims field-run mechanics and PRODUCT.md defines KPIs, but a serious adopter finds the claim, not the evidence | one public example repo aligned to the standard (drift 0, pinned, CI green) linked from the README, or an anonymized before/after case study of a real brownfield wave; PRODUCT.md notes how each KPI is observed, even manually | todo - **not closed by FIXTURE-1 or by the validation suite**: [`docs/validation/`](docs/validation/README.md) now publishes real cases run against 103 real repositories, with failures named alongside their fix - a large **read/dry-adoption** evidence base a sceptic can actually inspect, including its own disconfirmed claims. But every one of those repos was read, not aligned, and both fixtures (`test-greenfield-core`/`-node`) were written by the same people who wrote the standard, so the mechanics are demonstrated and nothing is proven about somebody else's repository. The evidence a sceptic needs - a real **L3/L4** adoption - still comes from FIELD-1, unchanged |
| FIXTURE-1 | A worked repository the skills can be run against | the skills are prompts and prompts have no test suite. Every mechanical guard has one; the procedures that produce most of the value have never been executed against a repo that was not being driven by their author, so "the wizard asks the right questions" is a design claim | a realistic adopting repo held outside this tree - filled product frame, roster, records, buildable specs, a populated pool, three closed cycles and one open, a timeline derived from them, and a git history whose dates and commit counts the artifacts actually match. Runnable repeatedly as the skills change | done (2026-08-02) - `sparrow-dispatch`, 19 commits, drift 0, all shipped guards green. Doubles as the demo material. **Produced one defect on its first run**: the fill warning matched generic notation, so a properly filled repo could never clear it (fixed, [`self-verify-fill-test`](tools/self-verify-fill-test.mjs)). Also caught a stale block in its own fixture data - the case `blocked:<id>` was built for |
