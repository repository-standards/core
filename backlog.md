...
| SKILL-COUNT-1 | open-question | Shipped skills | Decided: discriminability is the constraint, not the count - 22 ship today, reported rather than budgeted. A skill earns its place when its description names a situation no other one names | the doubt: every miss so far was a description that named no situation, and every fix was rewriting one with the count untouched - so nothing has yet tested whether the count can be the cause. It would take a miss that sharpening two descriptions cannot separate. (The drift risk across the four authoring skills is `AUTHORING-SKILLS-1` below, not this row.) [`shipped-skills`](docs/open-questions/shipped-skills.md) | decided |
