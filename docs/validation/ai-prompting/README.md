...
| `UPD-05` | update-to-version has a documented branch for a manifest `files` entry that changed upstream and also diverged locally - the actual shape of most merge- and fill-class entries, not only 'unchanged' or 'removed' | `fixture:test-greenfield-core` | re-verified in `runs/2026-08-06-x-tail.json` (no pull request cited) |
