...
| `DOC-13` | every surface that states the standard's current version is covered by facts-check, so a page saying '0.8.0' when the real version has moved on eleven times since is caught rather than shipped | `fixture:test-greenfield-core` | [core/pull/46](https://github.com/repository-standards/core/pull/46) |
