...
| `DEC-03` | the standard offers a way to defer to an existing, richer decision process instead of only filing legacy discovery material | `repo:rust-lang/rust` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
...
| `DEC-06` | idea-write is reachable by handoff from the two skills that claim to point at it, not only by name recognition | `fixture:test-greenfield-core` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
| `DEC-07` | graduating an approved idea that changes an existing capability routes to spec-update, not to spec-specify's new-capability path | `fixture:test-greenfield-core` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
...
| `DEC-09` | spec-impact's ripple search covers every artifact type the manifest requires, not only specs, ADRs and code | `fixture:test-greenfield-core` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
| `DEC-10` | a stale, now-false sentence in a spec whose Status is retired has a documented procedure to correct it, instead of every documented rule pointing a different, mutually-exclusive way | `fixture:test-greenfield-core` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
| `DEC-11` | the decision-record templates' author field is filled by the same convention self-verify's placeholder check already recognizes, not a second, uncovered mustache token | `fixture:test-greenfield-core` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
...
| `DEC-12` | the decision catalog has a category for a repo whose release gate is owned by a third party, not by its own CI or maintainer | `repo:Rdatatable/data.table` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
| `DEC-13` | the retroactive-decision reconstruction has a path for a founding decision that left no trace in the code or in the repo at all | `repo:opentofu/opentofu` | re-verified in `runs/2026-08-06-t-decisions.json` (no pull request cited) |
