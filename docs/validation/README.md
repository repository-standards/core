...
| Failures found | **159** - **89 fixed and re-verified** (across 24 merged pull requests), **70 still open right now** (of which 2 were attempted and a re-run found the fix did not fully hold), logged and named below, not hidden; 22 earlier fails superseded by a later re-run and no longer counted open |
