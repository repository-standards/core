...
| Failures found | **167** - **85 fixed and re-verified** (across 22 merged pull requests), **82 still open right now** (of which 3 were attempted and a re-run found the fix did not fully hold), logged and named below, not hidden; 9 earlier fails superseded by a later re-run and no longer counted open |
