...
| Failures found | **167** - **92 fixed and re-verified** (across 22 merged pull requests), **75 still open right now** (of which 1 were attempted and a re-run found the fix did not fully hold), logged and named below, not hidden; 15 earlier fails superseded by a later re-run and no longer counted open |
