...
[**results.md**](results.md), generated by `tools/human-prompting.mjs` from the run files.

Nothing computed them for the first three waves, and each one counted its own by hand. One of
those hand counts published `asked 27/33` where its own turns said 27/31, and three branches in
one week minted colliding prompt ids - one of them citing its own `A21` five times, so merging it
would have reattached a whole round's observations to a stranger's sentence. So the arithmetic
and the wiring are now checked: ids unique and contiguous, every observation citing a row that
exists, and a run whose prose states a fraction must agree with its own rows. What a verdict is
worth stays unmechanisable, which is the suite's premise and not a gap in it.
