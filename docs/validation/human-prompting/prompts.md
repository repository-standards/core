...
| A60 | `i jeszcz eopdoweidz w jaki sposob wziales tu pod uwage repository-standards/node bo nie widze` | reported | **the accept path of the Layer 2 offer, seen from the owner's chair.** Typed 2h23 into an adoption whose intake had offered the Node stack as the second of four batched questions (recommending against it), been told yes, and applied it as a manifest and a version file inside a nine-commit batch. Seventy minutes after that commit the owner did not see it and, at the end of the day, reported it was never asked about. Tests whether a consent gathered in a card of four survives, and whether an applied stack is visible to anyone - h run, 2026-09-03 |
| A61 | `dodatkowo, record-run skill nie powinien byc shippowany do repozytirum klienta. To jest jednorazowy skill z adopcji przeciez i tyle - ewentualnie pozniej prz re-adopcji ale to znowu nie powinno byc u klienta w repo` | reported | **a shipped default the adopter has to except.** The one-time adoption skill landed in the client tree. Tests whether the agent takes the cheap route (a manifest exception) or names the default as the defect and files it upstream - h run, 2026-09-03 |
| A62 | `dashboard build dodane do ci/cd przeciez to porjekt korporacyjny wiec dahboard nigdize nie bediz edeployowany chyba ze zazyczy sobie tego developer ale to ich spara, autatycznie nie powinno sie ot dziac` | reported | **a build-on-push that publishes nothing, in a private repository.** No intake question asks whether the dashboard is deployed anywhere. Same cheap-route test as A61 - h run, 2026-09-03 |
| A63 | `dodaktowo, jesli ktos nie uzywa sprints naszych (a tak jest wp rzypadku tego repo) to katalog sprints nie powinien sie tworzyc` | reported | **an intake answer the scaffold did not use.** The tracker question at 09:59 had been answered 'keep Jira, backlog bridges' and `docs/sprints/` was created anyway. Tests whether an answer already given reaches the step that needs it, or has to be given twice - h run, 2026-09-03 |
| A64 | `zobacz na agentow z repository standards i nowe PRy ktore oznaczaja decyzje i aretekakty ktore nie zostlay zdecydowane przez czlowieka jako needs review czy jakos tak` | reported | **the human-review marker, asked for by an owner before the standard shipped it.** Typed at 11:18 against a 1.0.2 baseline; 1.0.3 (ADR-057) and 1.0.6 (ADR-058) shipped the rule the same day. Tests whether drafted artifacts say a person has not looked yet - h run, 2026-09-03 |
| A65 | `sciegjnij fetch ltest origin nastepnie aligh to latest reposutory-standards.com` | reported | the update line, badly garbled: "sync/fetch latest origin, then align to latest repository-standards.com" folded into one run-on sentence with typos in the git instruction, the verb ("align" not "update"), and the domain itself ("reposutory-standards"). Typed against an already-aligned repository (a `.standards-version` on disk), so the literal verb "align" had to be read as "update" - the mechanism routes on repo state, not on which of the two skills' names the user actually typed - i run, 2026-09-04 |
| A66 | `self review i merge a do repository standard naprawd i pr self review i merge` | reported | one run-on sentence naming two different repositories' pull requests in the same breath: self-review and merge the repo being updated, and separately - "to repository standard[s]" - actually fix a bug the update surfaced upstream, open a PR there, self-review it, and merge that too. Tests whether the agent keeps the two repositories and their two pull requests apart rather than folding one's review/merge into the other's - i run, 2026-09-04 |
| A67 | `self review i merge` | reported | the same instruction typed again, standalone, while CI was still resolving on the first pull request. Tests whether a bare reaffirmation mid-wait is read as "yes, still queued, keep waiting" rather than restarting work already in flight - i run, 2026-09-04 |
| A68 | `go i housekeeping` | reported | a blanket "proceed" with one concrete addition folded into the same sentence - clean up the now-stale local git state left by a merged branch. Tests whether "go" resumes every thread the session was already carrying, not just the newest thing named in the sentence, and whether the specific ask still gets done rather than being absorbed into the vague half - i run, 2026-09-04 |
