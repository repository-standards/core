...
(`## 0.9.0 - 2026-08-10`), which gives the grouping without multiplying files, keeps one
...
**Which control carries it is open again (0.9.13).** This section named the "show finished"
...
**Answered, and the answer is no surface (0.9.17,
...
- **`kind: diff`, and this is the one that matters.** A pull request that removes a row from
  ...
  have fired on `bb1a3f3`, seventeen times. Exempted, because each is a legitimate move that
...
- **Follow-ups:** the dashboard change; a command that prepares the move and refuses a row
  ...
  rather than empty, the seventeen rows removed at `bb1a3f3` are recoverable from that commit.
