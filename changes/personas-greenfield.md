...
---
- `docs/personas.md` (+ ADR-006) - new: personas are a first-class **validation gate**, not a UX poster. They sit above specs and the backlog; every spec names the persona(s) it serves, every backlog item names whose job it advances, and an idea with no persona is parked. A persona-vs-persona conflict is resolved by a recorded BDR. Wired into the taxonomy, ways-of-working (the PO stage), the spec rule, the backlog template (a `persona` column), and the decision catalog (target personas = a BDR).
